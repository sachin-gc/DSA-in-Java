...
// Selection Sort: Selects the smallest (or largest) element in each iteration and swaps it with the current position, O(n²).
// Insertion Sort: Builds the sorted array one element at a time, O(n²) in the worst case.
// Quick Sort: Divide and conquer, average case O(n log n), worst case O(n²).
// Merge Sort: Divide and conquer, always O(n log n).
// Heap Sort: Uses a binary heap for sorting, O(n log n).
  }
}
